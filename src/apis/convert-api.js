    ...
    class ConvertApi {
        ...
        convert(fromFormat, toFormat, params) {
            return Promise.resolve(params.dto).then((dto) => {
                let altConvParam = dto.Parameters.filter(
                    (p) => p.Name.toLowerCase() == "converter",
                );
                let converterPath =
                    (altConvParam === null || altConvParam === void 0
                        ? void 0
                        : altConvParam.length) > 0
                        ? `/converter/${altConvParam[0].Value}`
                        : "";
                let auth = this.credentials.secret
                    ? `secret=${this.credentials.secret}`
                    : `apikey=${this.credentials.apiKey}&token=${this.credentials.token}`;
                return fetch(
                    `https://${this.host}/convert/${fromFormat}/to/${toFormat}${converterPath}?${auth}&storefile=true`,
                    {
                        method: "POST",
            ...
            if (this.file instanceof FileValue) {
                ...
                let response =
                    this.file instanceof URL
                        ? fetch(`${uploadUrl}url=${this.file.href}`, {
                              method: "POST",
                          })
                        : fetch(`${uploadUrl}filename=${this.file.name}`, {
                              method: "POST",
                              body: this.file,
                          });
    ...
    class FilesValue {
        ...
        asArray() {
            return this.files.map(
                (f) => new ConvertApi.FileValue(f.FileName, f.FileId),
            );
        }
    }
    ...
    class FilesParam {
        constructor(name, files, host) {
            ...
            if (files instanceof FileList) {
                this.fileValPros = Array.from(files).map((f) =>
                    new ConvertApi.FileParam(name, f, host)
                        .value()
                        .then((i) => ({
                            Id: i,
                        })),
                ...
                this.fileValPros = files.asArray().map((f) =>
                    Promise.resolve({
                        Id: f.fileId,
                    }),
                );
            } else {
                this.fileValPros = files.map((f) =>
                    Promise.resolve(f instanceof URL ? {Url: f.href} : {Id: f}),
                );
            }
        }
        get dto() {
...
(function (ConvertApi) {
    class Params {
                ...
                : init.forEach((p) => {
                      if (p.isFile) {
                          if (typeof p.value === "string") {
                              param = new ConvertApi.FileParam(
                                  p.name,
                                  new ConvertApi.FileValue("", p.value),
                                  this.host,
                              );
                          } else {
                              param =
                                  p.value instanceof Array
                                      ? new ConvertApi.FilesParam(
                                            p.name,
                                            p.value,
                                            this.host,
                                        )
        ...
        add(name, value) {
            let param;
            if (
                value instanceof ConvertApi.FilesValue ||
                value instanceof FileList ||
                value instanceof Array
            ) {
                param = new ConvertApi.FilesParam(name, value, this.host);
            } else if (
                value instanceof ConvertApi.FileValue ||
...
(function (ConvertApi) {
    class Result {
        ...
        toParamFile(idx = 0) {
            return new ConvertApi.FileValue(
                this.dto.Files[idx].FileName,
                this.dto.Files[idx].FileId,
            );
        }
        toParamFiles() {
...
(function (ConvertApi) {
    function worker(worker, params) {
        let paramPros =
            params instanceof HTMLFormElement
                ? Array.from(new FormData(params).entries()).map((pair) =>
                      resolveParam(pair[0], pair[1]),
                  )
                : Object.keys(params).map((k) => resolveParam(k, params[k]));
        return Promise.all(paramPros).then((p) =>
            fetch(worker.href, {
                method: "POST",
                headers: {
    ...
    function uploadMulti(value) {
        let dtoPros = Array.from(value)
            .filter((f) => f instanceof File)
            .map(upload);
        return Promise.all(dtoPros);
    }
    function upload(f) {
        return fetch(`https://v2.convertapi.com/upload?filename=${f.name}`, {
            method: "POST",
            body: f,
        })
            .then((resp) =>
                resp.status === 200
                    ? resp.json()
                    : Promise.reject(
                          `File ${f.name} upload has failed with the status code ${resp.status}`,
                      ),
            )
            .then((dto) => dto.FileId);
    }
...
export const ConvertFile = async (type, file, option = {secret: ""}) => {
    try {
        if (option?.secret) {
            const convertApi = ConvertApi.auth(option);
            ...
            console.log("result after convert => ", result, result.files[0]);
            return url;
        } else {
            return "wrong_secret";
        }
